        ...
        for k in self.rescales:
            if k == 4:
                self.upsample_4x = nn.Sequential(
                    nn.ConvTranspose2d(
                        embed_dim, int(embed_dim*scale_factor), kernel_size=2, stride=2),
                    build_norm_layer(norm_cfg, int(embed_dim*scale_factor))[1],
                    nn.GELU(),
                    nn.ConvTranspose2d(
                        int(embed_dim*scale_factor), int(embed_dim*scale_factor), kernel_size=2, stride=2),
                )
            elif k == 2:
                ...
                self.identity = nn.Conv2d(embed_dim, int(embed_dim*scale_factor), kernel_size=1)
            elif k == 0.5:
                self.downsample_2x = nn.Sequential(
                    nn.Conv2d(embed_dim, int(embed_dim*scale_factor), kernel_size=1),
